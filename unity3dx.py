...
#	print(response29.status_code)
	...
	if vast_system_type not in ["Liftoff", "{'version': '2.0'}", "KwaiNetwork", "InMobi","hivead","Bidstalk", "Temu", "unity", "{'version': '1.0', 'text': 'PubNative'}"]:
		print(vast_system_type)
	
	if vast_system_type in ["Liftoff", "KwaiNetwork", "unity", "hivead", "Temu", "{'version': '1.0', 'text': 'PubNative'}"]:
			
		vast_impression_url = vast_json["VAST"]["Ad"]["InLine"]["Impression"]
		
	if vast_system_type == "{'version': '2.0'}":
		
		vast_impression_url = vast_json["VAST"]["Ad"]["InLine"]["Impression"]["text"]
		
		
	if vast_system_type in ["InMobi", "Bidstalk"]:
			
		vast_impression_url = vast_json["VAST"]["Ad"]["Wrapper"]["Impression"]
		
		if vast_impression_url is not isinstance(vast_impression_url, list):
			response33 = requests.get(vast_impression_url, headers=headers33(vast_impression_url))
			print(response33.status_code)

		if vast_impression_url is isinstance(vast_impression_url, list):
			for vast_impression_url in vast_impression_url:
				response49 = requests.get(vast_impression_url, headers=headers33(vast_impression_url))
				print(response49.status_code)


		if vast_system_type == "InMobi":
			vast_Adtag_url = vast_json["VAST"]["Ad"]["Wrapper"]["VASTAdTagURI"]
			if vast_Adtag_url is not None:
				response47 = requests.get(vast_Adtag_url, headers=headers33(vast_Adtag_url))
				print(response47.status_code)
				root = ET.fromstring(response47.text)
				vast_json = {root.tag: element_to_dict(root)}
				vast_imp_array = vast_json["VAST"]["Ad"]["InLine"]["Impression"]
				for vast_imp_url in vast_imp_array:
					response48 = requests.get(vast_imp_url, headers=headers33(vast_imp_url))
					print(response48.status_code)
			
							

	if vast_system_type in ["Liftoff", "{'version': '2.0'}", "InMobi", "Bidstalk", "Temu", "{'version': '1.0', 'text': 'PubNative'}"]:
		
		vast_start_url = get_tracking_url(0, vast_json, "start")

	if vast_system_type == "KwaiNetwork":
		
		vast_start_url = get_tracking_url(1, vast_json, "start")
		
		if vast_start_url is not None and not isinstance(vast_start_url, list):
			response34 = requests.get(vast_start_url, headers=headers33(vast_start_url))
			print(response34.status_code)
			
		if isinstance(vast_start_url, list):
			print(vast_start_url)
...
#	print(response36.status_code)
	...
	if vast_system_type in ["Liftoff", "{'version': '2.0'}", "InMobi", "Bidstalk", "Temu", "{'version': '1.0', 'text': 'PubNative'}"]:
		...
		vast_firstQuartile_url = get_tracking_url(0, vast_json, "firstQuartile")
		
	if vast_system_type == "KwaiNetwork":
		
		vast_firstQuartile_url = get_tracking_url(1, vast_json, "firstQuartile")
		
		if vast_firstQuartile_url is not None and not isinstance(vast_firstQuartile_url, list):
			response38 = requests.get(vast_firstQuartile_url, headers=headers33(vast_firstQuartile_url))
			print(response38.status_code)
			
		if isinstance(vast_firstQuartile_url, list):
			print(vast_firstQuartile_url)
	...
	if vast_system_type in ["Liftoff", "{'version': '2.0'}", "InMobi", "Bidstalk", "Temu", "{'version': '1.0', 'text': 'PubNative'}"]:
		...
		vast_midpoint_url = get_tracking_url(0, vast_json, "midpoint")
		
	if vast_system_type == "KwaiNetwork":
		
		vast_midpoint_url = get_tracking_url(1, vast_json, "midpoint")
		
		if vast_midpoint_url is not None and not isinstance(vast_midpoint_url, list):
			response40 = requests.get(vast_midpoint_url, headers=headers33(vast_midpoint_url))
			print(response40.status_code)
			
		if isinstance(vast_midpoint_url, list):
			print(vast_midpoint_url)
	...
	if vast_system_type in ["Liftoff", "{'version': '2.0'}", "InMobi", "Bidstalk", "Temu", "{'version': '1.0', 'text': 'PubNative'}"]:
		...
		vast_thirdQuartile_url = get_tracking_url(0, vast_json, "thirdQuartile")
		
	if vast_system_type in ["unity"]:
		
		vast_thirdQuartile_url = get_tracking_url(0, vast_json, "progress")
		
		if vast_thirdQuartile_url is not None and not isinstance(vast_thirdQuartile_url, list):
			response42 = requests.get(vast_thirdQuartile_url, headers=headers33(vast_thirdQuartile_url))
			print(response42.status_code)
			
		if isinstance(vast_thirdQuartile_url, list):
			print(vast_thirdQuartile_url)
	...
	if vast_system_type in ["Liftoff", "{'version': '2.0'}", "InMobi", "Bidstalk", "Temu", "unity", "{'version': '1.0', 'text': 'PubNative'}"]:
		
		vast_complete_url = get_tracking_url(0, vast_json, "complete")
		
	if vast_system_type == "KwaiNetwork":
		
		vast_complete_url = get_tracking_url(1, vast_json, "complete")
		
		if vast_complete_url is not None and not isinstance(vast_complete_url, list):
			response44 = requests.get(vast_complete_url, headers=headers33(vast_complete_url))
			print(response44.status_code)
			
		if isinstance(vast_complete_url, list):
			print(vast_complete_url)
...
#	if vast_system_type in ["unity"]:
#		
#		vast_closeLinear_url = get_tracking_url(0, vast_json, "closeLinear")
#		
#		if vast_closeLinear_url is not None and not isinstance(vast_closeLinear_url, list):
#			response50 = requests.get(vast_closeLinear_url, headers=headers33(vast_closeLinear_url))
#			print(response50.status_code)


#	if vast_system_type in ["Liftoff", "InMobi", "unity"]:
#		
#		vast_creative_url = get_creative_url(vast_json, "creativeView")
#		
#	if vast_system_type in ["{'version': '1.0', 'text': 'PubNative'}"]:
#		
#		vast_creative_url = get_tracking_url(0, vast_json, "creativeView")
#		
#		if vast_creative_url is not None and not isinstance(vast_creative_url, list):
#			response46 = requests.get(vast_creative_url, headers=headers33(vast_creative_url))
#			print(response46.status_code)
#			
#		if isinstance(vast_creative_url, list):
#			print(vast_creative_url)